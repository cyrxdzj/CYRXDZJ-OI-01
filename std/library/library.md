...
选择好后，她会按顺序阅读每本书。阅读方式如上文所示，她会随机翻开一页，如果她的**头脑清醒度**大于等于被翻到的页码的**难度值**，那么她就可以看懂这页书。

**Miku 很关心真冬的学习情况，她希望知道，对于每一次拜访，真冬有多大的概率，翻开的所有 $r-l+1$ 页书页都可以被真冬看懂**。但 Miku 不擅长算数，因此她想请你帮忙。每当收到这样的信息，都请你的程序输出这个概率。
...
| 4~5   | $\le5\times10^4$ | $\le2\times10^4$ | $\le1\times10^5$ | $\le10$             | 书本不会更新，你只会收到消息 1             |
| 6~7   | $\le5\times10^4$ | $\le2\times10^4$ | $\le1\times10^5$ | $\le1\times10^5$    | 书本不会更新，你只会收到消息 1             |
...
| 样例1   | $\le50$          | $\le100$         | $\le300$         | $\le10$             | 不计分的样例，符合测试点 1~3 的要求         |
| 样例2   | $\le50$          | $\le100$         | $\le300$         | $\le10$             | 不计分的样例，符合测试点 1~3 的要求         |
| 样例3   | $\le5\times10^4$ | $\le2\times10^4$ | $\le1\times10^5$ | $\le1\times10^9$    | 不计分的样例，符合测试点 6~7 的要求与特殊性质    |
| 样例4   | $\le5\times10^4$ | $\le2\times10^4$ | $\le2\times10^5$ | $\le1\times10^5$    | 不计分的样例，符合测试点 14~17 的要求       |
| 样例5   | $\le5\times10^4$ | $\le2\times10^4$ | $\le2\times10^5$ | $\le1\times10^9$    | 不计分的样例，符合测试点 18~25 的要求       |

对于所有测试点，都有 $1\le n\le5\times10^4$，$1\le m\le2\times10^4$，$n\le C\le2\times10^5$，$1\le v,d_{i,j},d_{x,j}\le1\times10^9$，$1\le p_{i,j},p_{x,j}<998244353$。

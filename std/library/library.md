...
选择好后，她会按顺序阅读每本书。阅读方式如上文所示，对于每本书，她都会随机翻开一页（总计翻开 $r-l+1$ 页），如果她的**头脑清醒度**大于等于被翻到的页码的**难度值**，那么她就可以看懂这页书。
...
| 8~9   | $\le5\times10^4$ | $\le2\times10^4$ | $\le1\times10^5$ | $\le1\times10^9$    | 书本不会更新，你只会收到消息 1；$c_i\le2$   |
